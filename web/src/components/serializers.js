...
const serializers = {
  types: {
    ...
    image: ({ node }) => {
      return (
        <>
          <div className={`${node.fullwidth ? "w-full" : "w-6/12"} my-3 inline-block`}>
            <Figure node={node} altText={node.alt} />
          </div>
          <span className="block image-caption normal-case text-mobileCaption md:text-desktopCaption relative">
            {node.captionText}
          </span>
        </>
      );
    ...
    iframe: ({ node }) => {
      return (
        <>
          <div className="internal-iframe">{ReactHtmlParser(node.iframeCode)}</div>
          <span className="block image-caption normal-case text-mobileCaption md:text-desktopCaption relative">
            {node.iframeCaption}
          </span>
        </>
...
<path d="M44.8217 10.9321H43.2523V5.89843H37.2539V4.64646H43.2523V0H44.8217V10.9321Z" fill="black"/>
    ...
    articleImage: ({ node }) => (
      <div className="article-section-image">
        {node.caption ? <span className="article-image-caption">{node.caption}</span> : null}
        <Figure node={node} />
      </div>
    )
  },
  marks: {
    image: ({ mark, children }) => {
      return <img className="my-3" src={mark.asset.url} />;
    },
    ...
    circleLink: ({ mark, children }) => {
      if (mark.reference) {
        if (mark.reference._type == "home") {
          return (
            <CircleButton
              linkHome={mark.homeLink}
              linkRnd={mark.rndLink}
              linkHome={mark.homeLink}
              linkRnd={mark.rndLink}
              title={children}
              url={mark.reference}
    ...
    mapLink: ({ mark, children }) => {
      return <MapModule text={children} lat={mark.lat} long={mark.long} />;
    },
    contactPopup: ({ mark, children }) => {
      return (
        <>
          <ContactModal title={mark.title} subtitle={mark.subtitle}>
            {children}
          </ContactModal>
        </>
      );
    ...
    anchor: ({ mark, children }) => {
      return <span id={mark.anchorId}>{children}</span>;
    }
  }
};
