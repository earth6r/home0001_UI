...
export const PaneModules = ({ reactModule, type, specs = false }: { type: string; reactModule: any }) => {
  switch (type) {
    ...
    case "flexGallery":
      ...
      return (
        <>
          <FlexGallery squares={reactModule.squares} images={reactModule.images} rowNum={reactModule.numRows} rowNumMobile={reactModule.numRowsMobile} pdfs={reactModule.pdfs} embeds={reactModule.embeds} circleButtons={reactModule.circleButtons} />
          {reactModule.callibrationMark ? 
         <GridRow></GridRow>
          : <div className="w-full py-3"></div>}
        </>
